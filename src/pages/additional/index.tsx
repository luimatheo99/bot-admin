...
export function ListAdditional() {
  ...
  useEffect(() => {
    async function init() {
      const { data } = await api.get(`/restaurants/${user.idRestaurant}/additional`)
      setAdditional(data)
      setIsLoading(false)
    }
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 m-12">
        <div className="grid grid-cols-2 gap-2">
          <text className="text-2xl text-left">Adicionais</text>
          <div className="text-right">
            <button onClick={() => navigate('/additional/create')} type="button" className="focus:outline-none text-white font-medium rounded-md text-sm px-5 py-2.5 mb-2 bg-blue-700 hover:bg-blue-800 focus:ring-blue-300">
              Cadastrar
            </button>
          </div>
        </div>          
                  ...
                  {additional.map((additionalItem) => (
                    <tr className="bg-white hover:bg-gray-100 border-b border-gray-100">
                      <td className="text-center">
                        <button onClick={() => navigate(`/additional/edit/${additionalItem.id}`)} type="button" className="p-1 text-sm text-gray-500 focus:outline-none rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200">
                      ...
                      <td className="px-6 py-4 text-gray-500">
                        {new Intl.NumberFormat('pt-BR', {
                          style: 'currency',
                          currency: 'BRL',
                        }).format(Number(additionalItem.price))}
                      </td>
                    </tr>
                  ))}
                </tbody>
